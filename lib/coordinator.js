 ...
 * @param {Function} options.runOmniAnalysis - (prompt: string) => Promise<{ resultSummary: string, jobId?: string }>
...
function createCoordinator(options) {
   ...
   * @returns {Promise<{ reply: string, history: Array, omniJobId?: string }>}
   */
  async function chat(userMessage, conversationHistory = [], opts = {}) {
    ...
    while (iterations < maxIterations) {
      ...
      if (functionCalls.length === 0) {
        ...
        return { reply, history: contents, omniJobId: lastOmniJobId };
      }
      onEvent?.({
      ...
      const toolHandlers = {
        run_omni_analysis: async (args) => {
          ...
          const result = await runOmniAnalysis(prompt, { onEvent, mcpUserId: opts.mcpUserId });
          const summary = result?.resultSummary || result?.summary || String(result);
          lastOmniResultSummary = summary;
          if (result?.jobId) lastOmniJobId = result.jobId;
          onEvent?.({ type: 'tool_end', tool: 'run_omni_analysis', resultPreview: String(summary).slice(0, 200) });
          return { resultSummary: summary, querySpec: result?.querySpec || null, jobId: result?.jobId || null };
        },
    ...
    return {
      reply: 'I hit the maximum number of steps. Please try a shorter question.',
      history: contents,
      omniJobId: lastOmniJobId,
    };
  }
